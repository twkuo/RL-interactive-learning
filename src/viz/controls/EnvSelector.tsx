...
export function EnvSelector() {
  ...
  // Only show environments compatible with the current algorithm:
  // deep RL needs continuous (box) observations; tabular needs discrete ones.
  const needBox = getAlgoEntry(algoId).requires === 'box-obs';
  const envs = ENV_REGISTRY.filter((e) => (needBox ? e.obsKind === 'box' : e.obsKind !== 'box'));
  return (
