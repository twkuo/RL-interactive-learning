...
// Q(s,a) ← Q(s,a) + α[r + γ·Q(s',a')·(1−terminated) − Q(s,a)], where a' is the action actually taken next.
...
export class Sarsa implements Agent {
  readonly id = 'sarsa';
  readonly name = 'SARSA';
  readonly usesQ = true;
  ...
  update(t: Transition): UpdateInfo {
    ...
    const qNextSA = t.terminated || t.nextAction === undefined ? 0 : this.q.get(sN, t.nextAction);
    const target = t.reward + gamma * qNextSA;
